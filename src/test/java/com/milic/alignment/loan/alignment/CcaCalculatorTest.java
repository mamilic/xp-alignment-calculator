...
class CcaCalculatorTest {

  private final ClimateChangeAdaptationCalculator calculator =
      new ClimateChangeAdaptationCalculator();
  ...
  void should_calculateTotal_when_listIsEmpty() {
    ...
    Assertions.assertEquals(AmountType.CCA, result.getType());
  }
  ...
  void should_calculateTotal_when_alignmentKPIHasOneAndMultipleDataPoints() {
    ...
    // Then
    Assertions.assertEquals(Money.of(100, CurrencyCodes.EURO), result.getAligned());
    Assertions.assertEquals(Money.of(50, CurrencyCodes.EURO), result.getEligable());
    Assertions.assertEquals(AmountType.CCA, result.getType());
  }
